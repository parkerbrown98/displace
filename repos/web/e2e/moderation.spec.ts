...
test("reports a member and resolves the case through the moderator queue", async ({ page, request }, testInfo) => {
  ...
  await page.getByLabel("Action").selectOption("member.warn");
  await page.getByLabel("Reason code").first().selectOption("harassment");
  ...
  await page.getByLabel("Summary").fill("Warning issued and reporter notified.");
