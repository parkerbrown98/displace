...
function ChatMessage({ canManage, currentUserId, editing, message, onCancel, onDelete, onEdit, onSave }: { canManage: boolean; currentUserId?: string; editing: boolean; message: ChatMessageContract; onCancel: () => void; onDelete: () => void; onEdit: () => void; onSave: (messageId: string, body: string) => Promise<void> }) {
  const [draft, setDraft] = useState(message.body ?? "");
  const canChange = !message.isDeleted && (canManage || currentUserId === message.author.id);
  return <article className={`chat-message${message.isDeleted ? " deleted" : ""}`}><header><strong>{message.isDeleted ? "Deleted member" : message.author.displayName}</strong><span>@{message.author.handle}</span><time dateTime={message.createdAt}>{new Date(message.createdAt).toLocaleString()}</time>{canChange ? <span className="chat-message-actions"><button className="icon-button" onClick={onEdit} title="Edit message" type="button"><Edit3 size={14} /></button><button className="icon-button danger-button" onClick={onDelete} title="Delete message" type="button"><Trash2 size={14} /></button></span> : null}</header>{message.isDeleted ? <p className="tombstone">This message was removed.</p> : editing ? <form className="chat-edit-form" onSubmit={(event) => { event.preventDefault(); void onSave(message.id, draft.trim()); }}><textarea autoFocus maxLength={10_000} onChange={(event) => setDraft(event.target.value)} value={draft} /><div><button className="primary-button" disabled={!draft.trim()} type="submit">Save</button><button className="secondary-button" onClick={onCancel} type="button">Cancel</button></div></form> : <p>{message.body}</p>}</article>;
}