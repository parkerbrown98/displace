...
export function NotificationInbox() {
  ...
  useEffect(() => {
    ...
    let active = true;
    void listNotifications().then((page) => { if (active) setNotifications(page.items); }).catch((cause) => { if (active) setError(placeErrorMessage(cause, "Notifications could not be loaded.")); }).finally(() => { if (active) setLoading(false); });
  ...
  async function read(notification: NotificationContract) {
    ...
    catch (cause) { setError(placeErrorMessage(cause, "Notification could not be marked read.")); }
  }
