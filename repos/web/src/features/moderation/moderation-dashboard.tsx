    ...
    <main className="main-content moderation-page" id="main-content">
      ...
      {session.status === "loading" || loading && !placeId ? <LoadingPanel label="Loading moderation queue" /> : session.status !== "authenticated" ? <StatusPanel title="Sign in required" description="Sign in to review moderation cases." /> : places.length === 0 ? <StatusPanel title="No moderation access" description="None of your current place roles grant moderation management." /> : <>
        <div className="moderation-toolbar">
          <label className="form-field">Place<select onChange={(event) => { setLoading(true); setSelected(undefined); setChecked([]); setPlaceId(event.target.value); }} value={placeId}>{places.map(({ context }) => <option key={context.place.id} value={context.place.id}>{context.place.name}</option>)}</select></label>
          <label className="form-field">Status<select onChange={(event) => { setLoading(true); setSelected(undefined); setChecked([]); setStatus(event.target.value as ReportStatus); }} value={status}><option value="open">Open</option><option value="in_review">In review</option><option value="resolved">Resolved</option><option value="dismissed">Dismissed</option></select></label>
...
function BulkActionBar({ checked, onComplete, placeId, reports }: { checked: string[]; onComplete: () => Promise<void>; placeId: string; reports: ModerationReportContract[] }) {
  ...
  return <form className="bulk-action-bar" onSubmit={submit}><strong>{checked.length} selected</strong><label className="form-field">Reason code<select name="reasonCode" required>{actionReasonCodes.map((reason) => <option key={reason} value={reason}>{reason.replaceAll("_", " ")}</option>)}</select></label><label className="form-field grow">Warning<input maxLength={4000} name="reason" required /></label><button className="secondary-button" disabled={pending || selectedReports.length !== checked.length} title={selectedReports.length !== checked.length ? "Bulk warning is available for member reports" : undefined} type="submit"><Gavel size={15} />Warn members</button></form>;
}
...
function CaseDetail({ onChanged, placeId, report, userId }: { onChanged: () => Promise<void>; placeId: string; report: ModerationReportContract; userId: string }) {
  ...
  return <div className="case-detail">
    ...
    {report.status === "open" || report.status === "in_review" ? <>
      ...
      <section><h3>Action</h3><form className="case-action-form" onSubmit={action}><label className="form-field">Action<select name="action" required>{actions.map((name) => <option key={name} value={name}>{name.replaceAll(".", " ")}</option>)}</select></label>{report.targetType === "member" ? <label className="form-field">Timeout hours<input defaultValue={24} max={8760} min={1} name="durationHours" type="number" /></label> : null}{report.targetType === "topic" ? <label className="form-field">Destination forum ID<input name="targetForumId" /></label> : null}<label className="form-field">Reason code<select name="reasonCode" required>{actionReasonCodes.map((reason) => <option key={reason} value={reason}>{reason.replaceAll("_", " ")}</option>)}</select></label><label className="form-field wide">Reason<textarea maxLength={4000} name="reason" required rows={3} /></label><button className="danger-button" disabled={pending} type="submit"><Gavel size={16} />Apply action</button></form></section>
