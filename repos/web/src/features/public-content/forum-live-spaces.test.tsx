...
describe("ForumLiveSpaces", () => {
  ...
  it("links accessible chat and voice spaces with activity context", async () => {
    ...
    expect(screen.getByRole("link", { name: /Chat/ })).toHaveAttribute("href", "/places/game-makers/chat/lobby");
    expect(screen.getByText("#Lobby · 1 channel")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Voice/ })).toHaveAttribute("href", "/places/game-makers/voice");
    expect(screen.getByText("1 listening now")).toBeInTheDocument();
  });
});