...
export function RegisterPanel() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("sent");
    } catch (cause) { toast.error(authErrorMessage(cause, "Registration could not be completed. Review the form and try again.")); setState("editing"); }
  }
...
export function ForgotPasswordPanel() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    catch (cause) { toast.error(authErrorMessage(cause, "The request could not be sent. Try again.")); }
...
export function ResetPasswordPanel({ token }: { token?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!token) { toast.error("This reset link is incomplete or expired."); return; }
    const form = new FormData(event.currentTarget);
    const password = String(form.get("password") ?? "");
    if (password !== String(form.get("confirmation") ?? "")) { toast.error("Passwords do not match."); return; }
    setPending(true);
    try { await resetPassword(token, password); setComplete(true); }
    catch (cause) { toast.error(authErrorMessage(cause, "This reset link is invalid or expired.")); setPending(false); }
  }
